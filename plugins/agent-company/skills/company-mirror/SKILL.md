...
**Relays spend the mention chain.** A desk carries the hop of the last trigger it received, `penguin org channel send` from that desk is one hop deeper, and at the organization's `mention_chain_limit` (default 3) an `@` is recorded but no longer delivered. One question relayed out and one answer relayed back fit inside the default. A message from a human through the bound bot is not an organization trigger, so it does **not** reset that counter: a company that relays all day asks the board to raise `mention_chain_limit` in `org_config.toml` (an integer, picked up by the next reconcile, about 30 s). Never `@`-ping-pong to settle a detail — send one message carrying everything the other side needs.
