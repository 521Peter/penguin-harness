...
| `PENGUIN_API_HUB_ORIGIN` | Trusted origin used by the server-side Penguin API Hub key authorization flow | `https://token.penguin.ooo` |
...
`PENGUIN_API_HUB_ORIGIN` is server configuration, not a browser-supplied endpoint. It must be a bare HTTPS origin; plain HTTP is accepted only for `localhost`, `127.0.0.1`, or `[::1]` integration environments. Paths, credentials, query strings, and fragments are rejected at startup.
