...
| penguin-api-hub | `PENGUIN_API_HUB_API_KEY` | Preset relay group with fixed base URL `https://token.penguin.ooo/api`; its header can authorize a key automatically or set one manually |
...
TokenDance supports entering a one-time code when its redirect cannot return. Penguin API Hub is polled by the Models page and therefore has no manual-code mode.

Only the Project owner can start authorization. The full key never returns to the Web app. If writing a Penguin API Hub key locally fails, the server keeps that single delivery briefly so the write can be retried without authorizing again.
