...
- **先传话，再写下来。** 分身能从手册里答就直接答，答不了就把问题转给相关同事的分身，由它去问自己的真人再把答复带回来。带回来的东西一律写进手册（`people/<姓名>.md`、`faq.md`），下次同样的问题就能自己答掉。传话会消耗提及连锁，所以传话量大的镜像组织会把 `org_config.toml` 里的 `mention_chain_limit` 调高。
