 ...
 * bodies are validated before the service is asked; and the calling session and employee ride
 * write bodies as `sessionId` / `agentId` (a read's query string), but only from the control
 * environment's API token — a signed-in member's claim is dropped. The service itself is a recording fake here — its semantics have their
