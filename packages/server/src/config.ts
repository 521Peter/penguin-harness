...
export interface ServerConfig {
  ...
  /** Trusted Penguin API Hub origin; never supplied by a browser request. */
  penguinApiHubOrigin: string;
  /**
...
export function normalizePenguinApiHubOrigin(raw: string | undefined): string {
  ...
  try {
    ...
    throw new Error(
      `Invalid PENGUIN_API_HUB_ORIGIN=${value} (expected an absolute HTTP(S) origin)`,
    );
  }
  if (
    url.username !== "" ||
    ...
    throw new Error(
      `Invalid PENGUIN_API_HUB_ORIGIN=${value} (paths, credentials, query and fragment are not allowed)`,
    );
  }
  const loopback =
  ...
  if (url.protocol !== "https:" && !(url.protocol === "http:" && loopback)) {
    throw new Error(
      `Invalid PENGUIN_API_HUB_ORIGIN=${value} (HTTPS required except for loopback HTTP)`,
    );
  }
  return url.origin;
}

/** Parses server config from environment variables (PORT / HOST / PENGUIN_HOME / PENGUIN_WEB_DIST / PENGUIN_WEB_DB / PENGUIN_PREVIEW_ORIGIN / PENGUIN_API_HUB_ORIGIN / PENGUIN_SEED_ADMIN_PASSWORD / PENGUIN_DESKTOP_TOKEN / PENGUIN_PORT_FILE / PENGUIN_TRUST_PROXY / PENGUIN_CLI_ENTRY). */
export function resolveServerConfig(env: NodeJS.ProcessEnv = process.env): ServerConfig {
  ...
  return {
    ...
    penguinApiHubOrigin: normalizePenguinApiHubOrigin(env.PENGUIN_API_HUB_ORIGIN),
